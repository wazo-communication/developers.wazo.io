...
if (typeof window !== 'undefined') {
  ...
  const initSoftphone = () => {
    ...
    softphone.onIFrameLoaded = () => {
      document.getElementById('iframe-loaded-event').innerText =
        'Softphone iframe is loaded';
    };

    softphone.onAuthenticated = (session) => {
      document.getElementById('authenticated-event').innerText =
        `User authenticated, token: ${session.token}`;
    };
    ...
    softphone.onCardCanceled = () => {
      document.getElementById('card-canceled-event').innerHTML =
        'Card canceled 🙁';
    };

    softphone.onDisplayLinkedOption = (optionId) => {
      document.getElementById('display-linked-option-event').innerHTML =
        `Selected identified: ${optionId}`;
    };

    softphone.onWazoContactSearch = (query) => {
      document.getElementById('wazo-search-contact-event').innerHTML =
        `Searching for: ${query}`;
    };

    softphone.onAgentLoggedIn = () => {
      document.getElementById('agent-logged-in-event').innerHTML =
        'Agent logged in !';
    };

    softphone.onLanguageChanged = (language) => {
      document.getElementById('language-changed-event').innerHTML =
        `Language changed: ${language} !`;
    };

    softphone.onCallHeld = () => {
      document.getElementById('call-held-event').innerHTML =
        'Current call is held ⏸️';
    };

    softphone.onCallResumed = () => {
      document.getElementById('call-resumed-event').innerHTML =
        'Current call is resumed ▶️️';
    };

    softphone.onCallMuted = () => {
      document.getElementById('call-muted-event').innerHTML =
        'Current call is muted 🤐';
    };

    softphone.onCallUnMuted = () => {
      document.getElementById('call-unmuted-event').innerHTML =
        'Current call is un-muted 🔊';
    };

    softphone.onDtmfSent = (tone) => {
      document.getElementById('call-dtmf-event').innerHTML =
        `DTMF sent: ${tone}`;
    };

    softphone.onDirectTransfer = (number) => {
      document.getElementById('call-direct-transfer-event').innerHTML =
        `Call directly transferred to: ${number}`;
    };

    softphone.onCreateIndirectTransfer = (number) => {
      document.getElementById('call-indirect-transfer-event').innerHTML =
        `Indirect transfer created to: ${number}`;
    };
    ...
    softphone.onCancelIndirectTransfer = () => {
      document.getElementById('call-indirect-transfer-cancel-event').innerHTML =
        'Indirect transfer canceled';
    };

    softphone.onConfirmIndirectTransfer = () => {
      document.getElementById(
        'call-indirect-transfer-confirm-event',
      ).innerHTML = 'Indirect transfer confirmed';
    };

    softphone.onStartRecording = () => {
      document.getElementById('call-start-record-event').innerHTML =
        'Current call recorded';
    };

    softphone.onStopRecording = () => {
      document.getElementById('call-stop-record-event').innerHTML =
        'Current record stopped';
    };

    softphone.onCallLogCreated = (callLog) => {
      document.getElementById('call-log-event').innerHTML =
        `A new call log was created, its duration: ${callLog.duration}`;
    };

    softphone.onWebsocketMessage = (message) => {
      document.getElementById('websocket-event').innerHTML =
        `A new websocket message was received: ${JSON.stringify(message)}`;
    };
  ...
  const updateSoftphone = (
    extra = { right: 'auto', top: 'auto', left: 0, bottom: 0 },
    server = null,
  ) => {
  ...
  global.initButtons = () => {
    ...
    // Display
    document
      .querySelector('#display-softphone')
    ...
    document
      .querySelector('#customize-appearance')
      .addEventListener('click', (e) => {
        e.preventDefault();
        if (!displayed) {
          updateSoftphone();
      ...
      softphone.setFormSchema(
        {
          ...
          properties: {
            subject: {
              type: 'string',
              enum: ['Support', 'Greetings', 'Want to talk to Bob'],
              title: 'Subject',
            },
    ...
    document.querySelector('#advanced-form').addEventListener('click', (e) => {
      ...
      softphone.onOptionsResults('clientId', [
        { label: 'Alice', id: 1 },
        { label: 'Bob', id: 2 },
        { label: 'Charlies', id: 3 },
      ]);
